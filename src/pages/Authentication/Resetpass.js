...
function Forget() {
  ...
  const Forget = () => {
    ...
    dataArray.append("email", sessionStorage.getItem("email"))
    dataArray.append("otp", sessionStorage.getItem("emailOtp"))
    dataArray.append("newpassword", user.newpassword)
    dataArray.append("confirmpassword", user.confirmpassword)
    axios
      .post("http://103.186.185.77:5021/api/v1/admin/resetpassword", dataArray)
      .then(
        res => {
          if (res.status === 200) {
